...
# ============================================================
    ...
    with col_chart:
        ...
        fig_compare = px.scatter(
            ...
            labels={compare_var: compare_var, '메달점수': '메달 점수 (금10k/은500/동40)'}
        )
...
# ============================================================
    ...
    if st.button("🔮 메달 수 예측하기", type="primary", use_container_width=True):
        ...
        # 가중치: 금 10,000 / 은 500 / 동 40
        gold_medals = int(predicted_medals // 10000)
        remaining = predicted_medals % 10000
        silver_medals = int(remaining // 500)
        remaining2 = remaining % 500
        bronze_medals = int(remaining2 // 40)
        ...
        with col_result3:
            # 순위 추정 (메달점수 기준)
            rank = (df['메달점수'] > predicted_medals).sum() + 1
            st.metric("🏆 예상 순위", f"{rank}위",
                     help="베이징 2022 메달점수 기준")
